...
export function DataQualityClient({ initialData }: DataQualityClientProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex items-center justify-between gap-4 flex-wrap bg-gray-50 dark:bg-slate-900/60 p-5 rounded-2xl border border-border/50 backdrop-blur-md shadow-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-xl font-black text-gray-800 dark:text-slate-100 mt-1">مركز فحص جودة البيانات والدمج الذكي</h2>
        ...
        <button
          ...
          disabled={isPending}
          className="bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-200 border border-gray-200 dark:border-border/60 rounded-xl px-4 py-2.5 text-xs font-bold flex items-center gap-2 transition-all disabled:opacity-60"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-50 dark:bg-slate-900/60 border border-border/50 rounded-2xl p-4 backdrop-blur-md flex items-center justify-between shadow-lg">
          <div>
            <span className="text-xs text-gray-500 dark:text-slate-400 font-bold block">مجموعات التكرار المشتبه بها</span>
        ...
        <div className="bg-gray-50 dark:bg-slate-900/60 border border-border/50 rounded-2xl p-4 backdrop-blur-md flex items-center justify-between shadow-lg">
          <div>
            <span className="text-xs text-gray-500 dark:text-slate-400 font-bold block">الأسر ذات البيانات الناقصة</span>
        ...
        <div className="bg-gray-50 dark:bg-slate-900/60 border border-border/50 rounded-2xl p-4 backdrop-blur-md flex items-center justify-between shadow-lg">
          <div>
            <span className="text-xs text-gray-500 dark:text-slate-400 font-bold block">مؤشر سلامة قاعدة البيانات</span>
      ...
      {/* Tabs */}
      <div className="flex items-center gap-2 bg-gray-50 dark:bg-slate-900/60 p-1.5 rounded-xl border border-gray-100 dark:border-border/40 w-fit">
        <button
      ...
      {activeTab === "duplicates" && (
        <div className="space-y-4">
          ...
          data?.duplicateNameGroups?.length === 0 ? (
            <div className="bg-gray-50 dark:bg-slate-900/60 border border-border/50 rounded-2xl p-12 text-center text-gray-500 dark:text-slate-400 space-y-3">
              <CheckCircle2 className="h-12 w-12 mx-auto text-emerald-400 opacity-60" />
              <p className="text-base font-bold text-gray-700 dark:text-slate-200">ممتاز! لا توجد سجلات مكررة في قاعدة البيانات حالياً</p>
              <p className="text-xs text-gray-400 dark:text-slate-500">جميع أرقام الهويات الوطنية وأرقام الهواتف فريدة ونظيفة</p>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              {[...(data?.duplicateIdGroups || []), ...(data?.duplicatePhoneGroups || []), ...(data?.duplicateNameGroups || [])].map(
                (group: any, idx: number) => (
                  <div
                    key={idx}
                    className="bg-gray-50 dark:bg-slate-900/60 border border-amber-500/30 rounded-2xl p-5 backdrop-blur-md space-y-4 shadow-xl"
                  >
                    <div className="flex items-center justify-between border-b border-border/40 pb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-extrabold text-gray-800 dark:text-slate-100 font-mono bg-gray-100 dark:bg-slate-950 px-2 py-0.5 rounded border border-border/40">
                          {group.value}
                        </span>
                      </div>
                      ...
                      {group.items.map((item: any, iIdx: number) => (
                        <div
                          ...
                          className={`p-4 rounded-xl border space-y-3 text-xs ${
                            iIdx === 0
                              ? "bg-white dark:bg-slate-950/80 border-emerald-500/40 relative"
                              : "bg-white dark:bg-slate-950/40 border-border/50"
                          }`}
                        >
                          {iIdx === 0 && (
                          ...
                          <div className="flex justify-between items-start pt-1">
                            <div>
                              <div className="font-bold text-gray-800 dark:text-slate-100 text-sm">{item.headFullName}</div>
                            ...
                            <span className="font-mono text-[10px] text-gray-400 dark:text-slate-500 bg-gray-100 dark:bg-slate-900 px-2 py-0.5 rounded border border-border/30">
                              ID: {item.id.slice(-6).toUpperCase()}
                            </span>
                          </div>

                          <div className="text-[11px] text-gray-500 dark:text-slate-400 pt-2 border-t border-border/30">
      ...
      {activeTab === "incomplete" && (
        <div className="bg-gray-50 dark:bg-slate-900/60 border border-border/50 rounded-2xl overflow-hidden backdrop-blur-md shadow-xl">
          {data?.incompleteFamilies?.length === 0 ? (
            <div className="p-12 text-center text-gray-500 dark:text-slate-400 space-y-2">
              <CheckCircle2 className="h-12 w-12 mx-auto text-emerald-400 opacity-60" />
              <p className="text-base font-bold text-gray-700 dark:text-slate-200">جميع سجلات الأسر تشتمل على كافة البيانات الأساسية</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-right text-xs">
                <thead className="bg-white dark:bg-slate-950/80 text-gray-500 dark:text-slate-400 border-b border-gray-200 dark:border-border/60 font-bold">
                  <tr>
                    <th className="p-3">#</th>
                ...
                <tbody className="divide-y divide-border/40">
                  {data?.incompleteFamilies?.map((item: any, idx: number) => (
                    <tr key={item.family.id} className="hover:bg-gray-100 dark:hover:bg-slate-800/30 transition-all">
                      <td className="p-3 font-mono text-gray-500 dark:text-slate-400 font-bold">{idx + 1}</td>
                      <td className="p-3 font-bold text-gray-800 dark:text-slate-100">{item.family.headFullName}</td>
                      <td className="p-3 font-mono text-gray-600 dark:text-slate-300">{item.family.headNationalId}</td>
                      <td className="p-3">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {item.missingMembers && (
                            <span className="bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-400 border border-border/40 px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1">
